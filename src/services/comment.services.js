...
const isInputDataValid = (req, res) => {
    if (req.body.text.length > 1000) {
        res.status(400).send({message: "Text length should be less than or equal to 1000 characters."});
        return false;
    }
    return true;
}
...
const canUserEditComment = async (req, res) => {
    ...
    const isUserSuspendedInThisOrganization = await SuspendedUserRepository.isUserSuspended(req.userId, organizationId);
