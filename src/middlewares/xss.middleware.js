...
function sanitizeUserInput(req, res, next) {
  if (req.query) {
    if (containsDangerousInput(req.query)) {
      return res.status(400)
          .send(
              'Bad Request: Detected potential XSS vulnerability in query parameters.');
    }
  }
  if (req.body) {
    if (containsDangerousInput(req.body)) {
      return res.status(400)
          .send(
              'Bad Request: Detected potential XSS vulnerability in request body.');
    }
  }
  ...
  if (req.params) {
    if (containsDangerousInput(req.params)) {
      return res.status(400)
          .send(
              'Bad Request: Detected potential XSS vulnerability in request parameters.');
    }
  }
